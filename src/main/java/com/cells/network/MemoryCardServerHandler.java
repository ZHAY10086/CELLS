...
/**
 * Server-side event handler that intercepts memory card interactions with CELLS interfaces.
...
public class MemoryCardServerHandler {
    ...
    public void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        // Check if this is one of our interfaces (Item, Fluid, Gas, or Essentia)
        TileEntity te = event.getWorld().getTileEntity(pos);
        if (te == null) return;

        boolean isOurInterface = te instanceof IInterfaceHost || te instanceof IPartHost;
