 ...
 *
 * TODO: Investigate export not working - reported that the part version doesn't export
 * while the tile (full block) version does. Both use FluidInterfaceLogic.onTick() for
 * export logic. The issue may be in:
 * - Grid tickable registration (IGridTickable should auto-register via TickManagerCache)
 * - Grid proxy setup (PartBasicState sets REQUIRE_CHANNEL but not idle power)
 * - Channel/power status not being detected correctly
 * - External fluid handler capability not being exposed correctly
 */
public class PartFluidExportInterface extends PartBasicState implements IGridTickable, IFluidInterfaceHost, FluidInterfaceLogic.Host {
    ...
    private boolean useMemoryCard(final EntityPlayer player) {
        ...
        final String name = "tile.cells.export_interface" + "." + this.logic.getTypeName();
